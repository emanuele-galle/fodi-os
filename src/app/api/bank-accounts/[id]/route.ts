...
import { updateBankAccountSchema } from '@/lib/validation'
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!existing) return NextResponse.json({ success: false, error: 'Conto non trovato' }, { status: 404 })
    ...
    console.error('[bank-account-update]', e)
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (e instanceof Error && e.message.startsWith('Permission denied'))
      return NextResponse.json({ success: false, error: e.message }, { status: 403 })
    console.error('[bank-account-delete]', e)
    return NextResponse.json({ success: false, error: 'Errore interno del server' }, { status: 500 })
  }
}
