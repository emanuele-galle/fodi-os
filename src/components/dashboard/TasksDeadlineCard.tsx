              ...
              return sections.map(section => (
                <div key={section.label}>
                  ...
                  {section.tasks.map(task => (
                    <button type="button" key={task.id} onClick={() => router.push(`/tasks?taskId=${task.id}`)} className="w-full text-left flex items-center justify-between py-2.5 px-3 -mx-1 rounded-xl hover:bg-secondary/50 active:bg-secondary/60 transition-colors cursor-pointer touch-manipulation">
