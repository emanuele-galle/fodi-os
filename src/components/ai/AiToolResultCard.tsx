...
function renderProjectList(data: { projects: Array<Record<string, unknown>>; total: number }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-xs text-muted-foreground font-medium">{data.total} progetti trovati</p>
      {data.projects.slice(0, 8).map((project: Record<string, unknown>) => (
        <div key={project.id as string} className="flex items-center gap-2 text-xs py-1 px-2 rounded-lg bg-background/50">
          ...
          {project.progress !== undefined ? (
            <span className="text-xs text-muted-foreground">{project.progress as number}%</span>
          ) : null}
...
function renderTaskList(data: { tasks: Array<Record<string, unknown>>; total: number }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-xs text-muted-foreground font-medium">{data.total} task trovati</p>
...
function renderLeadList(data: { leads: Array<Record<string, unknown>>; total: number }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-xs text-muted-foreground font-medium">{data.total} lead trovati</p>
...
function renderDealList(data: { deals: Array<Record<string, unknown>>; total: number }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-xs text-muted-foreground font-medium">{data.total} trattative</p>
...
function renderQuoteList(data: { quotes: Array<Record<string, unknown>>; total: number }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-xs text-muted-foreground font-medium">{data.total} preventivi</p>
...
function renderTicketList(data: { tickets: Array<Record<string, unknown>>; total: number }) {
  ...
  return (
    <div className="space-y-1.5">
      <p className="text-xs text-muted-foreground font-medium">{data.total} ticket</p>
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground font-medium">{data.total || data.entries.length} registrazioni</p>
...
function renderGenericResult(data: unknown) {
  ...
  if (str.length > 500) return <pre className="text-xs text-muted-foreground whitespace-pre-wrap overflow-hidden max-h-40">{str.slice(0, 500)}...</pre>
  return <pre className="text-xs text-muted-foreground whitespace-pre-wrap">{str}</pre>
}
